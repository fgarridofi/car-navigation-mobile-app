...
class UserRepositoryImpl @Inject constructor( private val api: FormApi ): UserRepository {
    ...
    override suspend fun authenticateUser(loginModel2: LoginModel2): Result<Unit>{
        val response = try{
            api.authenticateUser(loginModel2)
        }catch (e: HttpException) {
            return Result.Error(UiText.StringResource(R.string.no_validate))
        } catch (e: IOException) {
            return Result.Error(UiText.StringResource(R.string.io_exception_error))
        }
        return Result.Success(data = response, message = UiText.StringResource(R.string.user_identificated_successfully))
